...
class G(object):
	...
	def __init__(self, outfile=None, print_lines=True, header=None, footer=None,
		...
		self._socket = None
		if "serial" in direct_write_mode and printer_port is not None:
	...
	def reset_home(self):
		...
		# current_position
		return self.write('G92.1')

	def homing(self):
		return self.write('G28')
...
UR
		...
		"""
		res = self.write('G1 F{}'.format(rate))
		self.speed = rate
		return res
	...
	def abs_move(self, x=None, y=None, z=None, **kwargs):
		...
		if self.is_relative:
			self.absolute()
			res = self.move(x=x, y=y, z=z, **kwargs)
			self.relative()
		else:
			res = self.move(x=x, y=y, z=z, **kwargs)
		return res
	...
	def write(self, statement_in, resp_needed=True):
		if self.print_lines:
			print(statement_in)
